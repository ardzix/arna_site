...
class DeepSeekAdapter:
    ...
    def generate_fe_guide(self, template_payload):
        ...
        prompt = (
            'Generate a valid JSON object for FE guide in template creation context. '
            'Return JSON only with fields required by fe-guide.schema.json.\n'
            'Important intent:\n'
            '1) Focus on frontend implementation guideline, not business copywriting.\n'
            '2) Explain component architecture, HTML semantics, CSS layout system, responsive behavior, and UX interaction patterns.\n'
            '3) Avoid discussing textual content strategy in detail (headlines/marketing copy).\n'
            '4) `section_catalog` must describe each section purpose from UI structure perspective.\n'
            '5) `component_mapping` must map section type to reusable FE components and prop contracts.\n'
            '6) `implementation_notes` must be practical engineering checklist items (accessibility, performance, state, edge cases).\n'
            '7) `markdown` must be long-form technical guide with clear headings:\n'
            '   - Architecture\n'
            ...
            '   - Theming (including dark mode and language switcher if present)\n'
            '   - QA Checklist\n'
            '8) Keep it framework-agnostic, but FE-implementable in React/Next.\n'
    ...
    def build_fe_guide_from_template(self, template_payload):
        ...
        for s_type in section_types:
            ...
            section_catalog.append({
                'type': s_type,
                'purpose': f'Provide {s_type} layout structure and interaction zone with reusable UI patterns.',
                'required_fields': ['title', 'description'],
                'optional_fields': ['subtitle', 'image_url', 'extra', 'items', 'order', 'is_active'],
            })
        ...
        markdown_lines = [
            ...
            "",
            "## Architecture",
            "- Use a section-driven renderer: each `page.sections[]` item is resolved by `section.type`.",
            "- Keep data mapping separated from visual components (adapter/mapping layer).",
            "- Build components as presentational units with stable prop contracts.",
            "",
            "## Section Mapping",
        ]
        for s_type in section_types:
        ...
        markdown_lines.extend([
            "",
            "## Component Responsibilities",
            "- Each section component handles structure/layout only, not API fetching.",
            "- Use small subcomponents for repeated elements (cards, badges, CTA buttons, list items).",
            "- Guard optional data (`subtitle`, `image_url`, `extra`) with safe defaults.",
            "",
            "## Layout and Spacing System",
            "- Use a consistent container width and section vertical rhythm.",
            "- Define spacing scale tokens (e.g., 4/8/12/16/24/32/48/64).",
            "- Keep visual hierarchy with predictable heading, body, and action zones.",
            "",
            "## Responsive Rules",
            "- Mobile-first layout, then scale to tablet and desktop.",
            "- Convert multi-column areas into single-column stack on small screens.",
            "- Prevent overflow for long text and dynamic list items.",
            "",
            "## Accessibility Checklist",
            "- Use semantic landmarks (`header`, `main`, `section`, `footer`).",
            "- Ensure heading order is logical and sequential.",
            "- Ensure all interactive controls are keyboard reachable.",
            "- Keep color contrast safe in both light and dark modes.",
            "",
            "## Interaction and Micro-UX",
            "- Keep hover/focus/active states visually distinct.",
            "- Add subtle transitions for section reveal and CTA interactions.",
            "- Preserve scroll position behavior during language/theme toggles.",
            "",
            "## Theming and Localization",
            "- Drive colors/typography from design tokens or CSS variables.",
            "- If `features.dark_mode_toggle=true`, implement class-based theme switch with persisted preference.",
            "- If `features.language_switcher=true`, isolate UI labels in translation dictionary keyed by locale.",
            "",
            "## QA Checklist",
            "- Sort pages/sections/blocks by `order` ascending.",
            "- Render only active entities (`is_active=true`).",
            "- Validate empty/partial data does not break layout.",
            "- Verify dark mode and language switcher on mobile and desktop.",
            "",
            "## Implementation Notes",
            "- Keep component props resilient to optional fields.",
            "- Prefer deterministic rendering paths over implicit assumptions.",
        ])
        ...
        return {
            ...
            'implementation_notes': [
                ...
                'Implement semantic HTML and keyboard-accessible interactions.',
                'Use tokenized spacing, typography, and color system for consistency.',
                'Support dark mode and language switcher through centralized UI state.',
            ],
            'markdown': '\n'.join(markdown_lines),
        }
